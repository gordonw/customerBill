...
class CustomerBillSpec extends GebSpec {
	...
	def "Customer bill contains Sky package details"(){
		...
		assert packageDetails[0].name == 'Variety with Movies HD'
		assert packageDetails[0].type == 'tv'
		assert packageDetails[0].cost == '£50.00'

		assert packageDetails[1].name == 'Sky Talk Anytime'
		assert packageDetails[1].type == 'talk'
		assert packageDetails[1].cost == '£5.00'

		assert packageDetails[2].name == 'Fibre Unlimited'
		assert packageDetails[2].type == 'broadband'
		assert packageDetails[2].cost == '£16.40'
